...
module.exports = function(homebridge)
{
  ...
  homebridge.registerPlatform("homebridge-onkyo", "Onkyo", OnkyoPlatform, true);
}

class OnkyoPlatform extends HomebridgePlatform {
	...
	addAccessory(accessory) {
		this.api.registerPlatformAccessories("homebridge-onkyo", "Onkyo", [accessory]);
	}

	discoverOnkyoAccessories() {
...
class OnkyoAccessory {
	...
	getVolumeState(callback, context) {
		...
		this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["volume"] + "=query", function( error, data) {
			if (error) {
				...
				if (that.dimmer ) {
					that.dimmer.getCharacteristic(Characteristic.Brightness).setValue(that.v_state, null, "v_statuspoll");
				}
			}
		}.bind(this) );
	}
	...
	getMuteState(callback, context) {
		...
		this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["muting"] + "=query", function( error, data) {
			if (error) {
				...
				if (that.dimmer) {
					that.dimmer.getCharacteristic(Characteristic.On).setValue(that.m_state, null, "m_statuspoll");
				}
			}
		}.bind(this) );
	}
	...
	prepareVolumeDimmer() {
		this.log.info("Setting up dimmer")
