...
class OnkyoAudioReceiverAccessory {
    constructor(platform, accessory, config, log) {
        ...
        this.buttons = {
            ...
            [this.platform.Characteristic.RemoteKey.ARROW_UP]: 'up',
            [this.platform.Characteristic.RemoteKey.ARROW_DOWN]: 'down',
            [this.platform.Characteristic.RemoteKey.ARROW_LEFT]: 'left',
            [this.platform.Characteristic.RemoteKey.ARROW_RIGHT]: 'right',
            [this.platform.Characteristic.RemoteKey.SELECT]: 'enter',
            [this.platform.Characteristic.RemoteKey.BACK]: 'exit',
            [this.platform.Characteristic.RemoteKey.EXIT]: 'exit',
            [this.platform.Characteristic.RemoteKey.PLAY_PAUSE]: 'play',
        ...
        this.eiscp.connect({ host: this.ipAddress, reconnect: true, model: this.model });
        this.setUp();
    }
    setUp() {
        this.createRxInput();
    ...
    polling(accessory) {
        ...
        if (this.switchHandling == "poll") {
            ...
            var statusemitter = pollingtoevent((done) => {
                ...
                this.getPowerState((error, response) => {
            ...
            // Audio-Input Polling
            var i_statusemitter = pollingtoevent((done) => {
            ...
            var m_statusemitter = pollingtoevent((done) => {
                this.log.debug("start MUTE polling..");
                this.getMuteState((error, response) => {
                    //pass also the setAttempt, to force a homekit update if needed
                    done(error, response, this.setAttempt);
                }, "m_statuspoll");
            ...
            // Volume Polling
            var v_statusemitter = pollingtoevent((done) => {
                this.log.debug("start VOLUME polling..");
                this.getVolumeState((error, response) => {
                    //pass also the setAttempt, to force a homekit update if needed
                    done(error, response, this.setAttempt);
                }, "v_statuspoll");
    ...
    setPowerState(powerOn, callback, context) {
        ...
        if (powerOn) {
            ...
            this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["power"] + "=on", (error, response) => {
                ...
                if (error) {
                    ...
                    if (powerOn && this.defaultVolume) {
                        ...
                        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["volume"] + ":" + this.defaultVolume, (error, response) => {
                            if (error) {
                    ...
                    const index = label !== null
                        ? RxInputs.Inputs.findIndex((i) => i.label == label)
                        : -1;
                    this.i_state = index + 1;
                    if (powerOn && label) {
                        this.log.info("Setting default input selector to " + label);
                        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["input"] + "=" + label, (error, response) => {
                            if (error) {
            ...
            this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["power"] + "=standby", (error, response) => {
    ...
    getPowerState(callback, context) {
        ...
        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["power"] + "=query", (error, data) => {
            if (error) {
                this.state = false;
    ...
    getVolumeState(callback, context) {
        ...
        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["volume"] + "=query", (error, data) => {
            if (error) {
                this.v_state = 0;
    ...
    setVolumeState(volumeLvl, callback, context) {
        ...
        //have the event later on execute changes
        callback(null, this.v_state);
        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["volume"] + ":" + this.v_state, (error, response) => {
            if (error) {
                this.v_state = 0;
    ...
    setVolumeRelative(volumeDirection, callback, context) {
        ...
        if (volumeDirection == this.platform.Characteristic.VolumeSelector.INCREMENT) {
            ...
            this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["volume"] + ":" + "level-up", (error, response) => {
                if (error) {
                    this.v_state = 0;
            ...
            this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["volume"] + ":" + "level-down", (error, response) => {
                if (error) {
                    this.v_state = 0;
                    this.log.error("setVolumeRelative - VOLUME : ERROR - current v_state: %s", this.v_state);
                }
    ...
    getMuteState(callback, context) {
        ...
        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["muting"] + "=query", (error, data) => {
            if (error) {
                this.m_state = false;
    ...
    setMuteState(muteOn, callback, context) {
        ...
        if (this.m_state) {
            ...
            this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["muting"] + "=on", (error, response) => {
                if (error) {
                    this.m_state = false;
            ...
            this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["muting"] + "=off", (error, response) => {
                if (error) {
                    this.m_state = false;
    ...
    getInputSource(callback, context) {
        ...
        this.eiscp.command(this.zone + "." + this.cmdMap[this.zone]["input"] + "=query", (error, data) => {
            if (error) {
                this.i_state = 1;
    ...
    remoteKeyPress(button, callback) {
        ...
        if (this.buttons[button]) {
            ...
            this.eiscp.command(this.zone + "." + "setup=" + press, (error, data) => {
                if (error) {
                    // this.i_state = 1;
    ...
    setupInput(inputCode, name, hapId, television) {
        const input = new this.platform.Service.InputSource(`${this.name} ${name}`, inputCode);
    ...
    createTvService() {
        ...
        const tvService = new this.platform.Service.Television(this.name, "AUDIO_RECEIVER");
        tvService
