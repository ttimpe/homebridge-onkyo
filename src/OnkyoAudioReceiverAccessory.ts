	...
	polling(platform: OnkyoAudioReceiverAccessory) {
		...
		if (that.switchHandling === 'poll') {
			...
			v_statusemitter.on('v_statuspoll', (data: any) => {
				...
				// if (that.tvService ) {
					// 	that.tvService.getCharacteristic(this.Characteristic.Volume).updateValue(that.v_state, null, 'v_statuspoll');
					// }
				});
		}
	}
		...
		setPowerState(powerOn: any, callback: CharacteristicSetCallback, context: string) {
			...
			addSources() {
				...
				const inputs = RxInputs.Inputs.map((i: any, index: number) => {
					...
					const input = this.setupInput(i.code, inputName, hapId);
					return input;
				});
				return inputs;
			}
			...
			createTvService() {
				...
				this.tvService = new this.Service.Television(this.name, 'AUDIO_RECEIVER');
			...
			createTvSpeakerService() {
				this.tvSpeakerService = new this.Service.TelevisionSpeaker(this.name)
